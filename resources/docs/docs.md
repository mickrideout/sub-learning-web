...
The `home-routes` handler in the `sub-learning-web.routes.home` namespace
...
The `home-page` function will in turn call the `sub-learning-web.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `sub-learning-web.handler` namespace:
...
Request middleware functions are located under the `sub-learning-web.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `sub-learning-web.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `sub-learning-web.db.core` in some other namespace.
